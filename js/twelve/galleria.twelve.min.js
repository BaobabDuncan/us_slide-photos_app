...
this.bind("image", function(e) {
    ...
    var data = this._data;    
    var favorites_key = data[active].favorites_key;

    
    //$("#favorites_key").val(favorites_key);    
    
    
    //$("#image_title").html(data[active].description.substr(0, 50));
    //$("#image_created").html(data[active].created);
    var html = '';
    html +='<div id="form_div">';
    html +='<form action="" >';
    html +='<input type="hidden" name="favorites_key" id="favorites_key" value="'+favorites_key+'">';   
    html +='<input type="submit" class="unfavorites_submit" title="Un Favorites" value="">';
    html +='</form>';
    html +='</div>';  
   
    
    $(".galleria-unfavorites-form").html(html);
});
